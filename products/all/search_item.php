<? include "../../config/core.php"; ?>
   ...
   <? if (isset($_GET['product_search'])): ?>
		<? $search = strip_tags($_POST['result']); ?>
         ...
         <? $item = db::query("select * from product_item where (article like '%$search%') or (barcode like '%$search%') order by ins_dt desc limit 30"); ?>
         ...
         <? else: ?>
            ...
            <? $product = db::query("select * from product where (name_ru like '%$search%') or (name_kz like '%$search%') order by ins_dt desc limit 10"); ?>
