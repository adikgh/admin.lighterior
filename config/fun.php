<? 
	...
	class fun {
		...
		public static function user_management2($id) {
			$sql = db::query("select * from user_management where id = '$id'");
			if (mysqli_num_rows($sql)) return mysqli_fetch_array($sql); else return 0;
		}
		public static function user_staff_name($id, $lang) {
			$sql = db::query("select * from user_staff where id = '$id'");
			if (mysqli_num_rows($sql)) return (mysqli_fetch_array($sql))['name_'.$lang]; else return 'null';
		}
